...
class DhCuttingPersonController extends Controller implements HasMiddleware
{
    ...
    {
        return [
            new Middleware('page.permission:dh_cutting_person,view', only: ['index', 'show']),
            new Middleware('page.permission:dh_cutting_person,edit', only: ['create', 'store', 'edit', 'update', 'quickStore']),
            new Middleware('page.permission:dh_cutting_person,remove', only: ['destroy']),
        ];
    }
