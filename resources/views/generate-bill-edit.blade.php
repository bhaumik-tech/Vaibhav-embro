        ...
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div class="w-full lg:w-1/3 bg-white border border-slate-200 p-4 shadow-sm flex flex-col justify-center gap-2">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <input type="date" name="date" value="{{ \Carbon\Carbon::parse($generateBill->date)->format('Y-m-d') }}" required class="bg-transparent border-none p-0 text-slate-900 font-bold text-lg focus:ring-0 w-full">
                </div>
            </div>
        </div>
    </div>
                ...
                @foreach($generateBill->items as $index => $item)
                    ...
                    <tr class="border-b border-slate-200 hover:bg-slate-50/50 group">
                        ...
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="text" name="items[{{$rowCount}}][ch_no]" value="{{ $item->ch_no }}" onblur="fetchChalanDetails(this)" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                                ...
                                @if(is_array($item->details) && count($item->details) > 0)
                                    @foreach($item->details as $dIndex => $detail)
                                        <div class="flex gap-2 items-center">
                                            ...
                                            <input type="text" name="items[{{$rowCount}}][details][{{$dIndex}}][value]" value="{{ $detail['value'] ?? '' }}" placeholder="#..." class="flex-1 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center min-w-0">
                                            @if($dIndex == 0)
...
<script>
    ...
    function addDetailRow(btn) {
        ...
        newRow.innerHTML = `
            ...
            <input type="text" name="items[${rIndex}][details][${dIndex}][value]" placeholder="#..." class="flex-1 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center min-w-0">
            <button type="button" onclick="this.closest('.flex').remove()" class="text-slate-500 hover:text-red-600 p-1.5 bg-slate-100 hover:bg-red-50 border border-slate-200 shrink-0 transition-colors" title="Remove Detail">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 12H4"/></svg>
            </button>
        `;
    ...
    else updateGrandTotal(); // Init total on load
</script>
@endsection
