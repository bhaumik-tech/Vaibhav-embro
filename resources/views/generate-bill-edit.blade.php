...
@section('content')
<form action="{{ route('generate-bills.update', $generateBill->id) }}" method="POST" class="bg-white shadow-sm border border-slate-200">
    @csrf
    @method('PUT')
    <!-- Form Header -->
        ...
        <div class="flex flex-col lg:flex-row gap-4">
            ...
            <div class="flex-1 bg-white border border-slate-200 p-4 shadow-sm relative">
                ...
                <div class="flex flex-col gap-2 w-full pt-10 sm:pt-0 sm:pr-48">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="text" name="add" id="party-add" value="{{ $generateBill->add }}" placeholder="Add" class="flex-1 font-medium text-slate-600 text-sm bg-transparent border-none p-0 focus:ring-0">
                    </div>
                    <div class="flex items-center gap-2">
                        <label class="font-semibold text-slate-700 w-12 text-sm uppercase">GST:</label>
                        <input type="text" name="gst" id="party-gst" value="{{ $generateBill->gst }}" placeholder="GST" class="flex-1 font-medium text-slate-600 text-sm bg-transparent border-none p-0 focus:ring-0">
                    </div>
                </div>
            </div>
            ...
            <div class="w-full lg:w-1/4 bg-white border border-slate-200 p-4 shadow-sm flex flex-col justify-center gap-3">
                <div class="border border-slate-200 bg-slate-50 p-2 flex flex-col gap-1 text-center">
                    ...
                    <input type="text" name="bill_no" value="{{ $generateBill->bill_no }}" placeholder="Auto" class="bg-white border border-slate-200 px-2 py-1 text-slate-900 font-bold text-lg focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full placeholder-slate-400 text-center">
                </div>
                <div class="border border-slate-200 bg-slate-50 p-2 flex flex-col gap-1 text-center">
                    <label class="font-semibold text-slate-500 text-[10px] uppercase tracking-wider">Date</label>
                    <input type="date" name="date" value="{{ \Carbon\Carbon::parse($generateBill->date)->format('Y-m-d') }}" required class="bg-white border border-slate-200 px-2 py-1 text-slate-900 font-bold text-lg focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-center">
                </div>
            </div>
        </div>
    </div>
                ...
                @foreach($generateBill->items as $index => $item)
                    ...
                    <tr class="border-b border-slate-200 hover:bg-slate-50/50 group">
                        <td class="px-2 py-2 border-r border-slate-200 align-top pt-4">
                            <input type="text" name="items[{{$rowCount}}][sr_no]" value="{{ $item->sr_no ?? $rowCount }}" class="row-number-input w-full border-none p-0 text-sm font-medium text-slate-900 text-center bg-transparent focus:ring-0">
                        </td>
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="text" name="items[{{$rowCount}}][ch_no]" value="{{ $item->ch_no }}" oninput="onChalanInput(this)" onblur="fetchChalanDetails(this)" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                                ...
                                @if(is_array($item->details) && count($item->details) > 0)
                                    @foreach($item->details as $dIndex => $detail)
                                        <div class="flex gap-2 items-center">
                                            ...
                                            <input type="text" name="items[{{$rowCount}}][details][{{$dIndex}}][value]" value="{{ '#' . ltrim($detail['value'] ?? '', '#') }}" class="flex-1 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center min-w-0 detail-code-input">
                                            @if($dIndex == 0)
    ...
    <div class="p-4 border-t border-slate-200 bg-white flex justify-end shrink-0">
        <div class="w-64 flex flex-col gap-2">
            <div class="flex items-center justify-between gap-2 border border-slate-200 p-2 rounded bg-slate-50">
                ...
                <input type="number" step="0.01" name="vatav_percent" value="{{ $generateBill->vatav_percent ?? 5.00 }}" class="w-24 border-none p-1 text-right font-bold text-slate-900 bg-white shadow-sm focus:ring-indigo-500 rounded">
            </div>
            <div class="flex items-center justify-between gap-2 border border-slate-200 p-2 rounded bg-slate-50">
                <label class="font-bold text-sm text-slate-700">SGST %</label>
                <input type="number" step="0.01" name="sgst_percent" value="{{ $generateBill->sgst_percent ?? 2.50 }}" class="w-24 border-none p-1 text-right font-bold text-slate-900 bg-white shadow-sm focus:ring-indigo-500 rounded">
            </div>
            <div class="flex items-center justify-between gap-2 border border-slate-200 p-2 rounded bg-slate-50">
                <label class="font-bold text-sm text-slate-700">CGST %</label>
                <input type="number" step="0.01" name="cgst_percent" value="{{ $generateBill->cgst_percent ?? 2.50 }}" class="w-24 border-none p-1 text-right font-bold text-slate-900 bg-white shadow-sm focus:ring-indigo-500 rounded">
            </div>
            <div class="flex items-center justify-between gap-2 border border-slate-200 p-2 rounded bg-slate-50">
                <label class="font-bold text-sm text-slate-700">TDS %</label>
                <input type="number" step="0.01" name="tds_percent" value="{{ $generateBill->tds_percent ?? 1.00 }}" class="w-24 border-none p-1 text-right font-bold text-slate-900 bg-white shadow-sm focus:ring-indigo-500 rounded">
            </div>
