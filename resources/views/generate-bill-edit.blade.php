@extends('layouts.app')
@section('title', 'Edit Generate Bill')

@section('content')
<div class="flex w-full h-full">
    <div id="form-container" class="w-full transition-all duration-300 h-full">
        <form action="{{ route('generate-bills.update', $generateBill->id) }}" method="POST" class="bg-white shadow-sm border border-slate-200 h-full overflow-y-auto">
    @csrf
    @method('PUT')
    <!-- Form Header -->
        ...
        <div class="relative bg-white border border-slate-300 shadow-sm rounded-md overflow-hidden hover:border-indigo-400 transition-colors">
            <select name="firm_id" required class="w-full border-none pl-4 pr-12 py-2.5 font-bold text-slate-800 text-center text-lg focus:ring-0 appearance-none bg-transparent cursor-pointer">
                ...
                @foreach($firms as $firm)
                    <option value="{{ $firm->id }}" {{ $generateBill->firm_id == $firm->id ? 'selected' : '' }}>{{ $firm->name }}</option>
                @endforeach
            </select>
            ...
            <div class="flex-1 bg-white border border-slate-300 shadow-sm relative rounded-md overflow-hidden flex flex-col">
                    ...
                    <div class="relative w-48">
                        <select name="party_id" required id="party-select" class="w-full text-xs font-bold text-slate-700 bg-white border border-slate-300 rounded shadow-sm px-2 py-1 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 appearance-none pr-6 cursor-pointer">
                            ...
                            @foreach($parties as $party)
                                <option value="{{ $party->id }}" data-name="{{ $party->name }}" data-add="{{ $party->address ?? '' }}" data-gst="{{ $party->gst_number ?? '' }}" data-vatav="{{ $party->vatav ?? 5.00 }}" data-sgst="{{ $party->sgst ?? 2.50 }}" data-cgst="{{ $party->cgst ?? 2.50 }}" data-tds="{{ $party->tds ?? 1.00 }}" {{ $generateBill->party_id == $party->id ? 'selected' : '' }}>{{ $party->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="p-4 flex flex-col gap-3 flex-1">
                    <div class="flex items-center gap-3">
                        ...
                        <input type="text" name="name" id="party-name" value="{{ $generateBill->name }}" placeholder="Name" class="flex-1 font-bold text-slate-800 text-sm bg-white border border-slate-300 shadow-sm rounded px-3 py-1.5 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div class="flex items-center gap-3">
                        <label class="font-bold text-slate-600 w-12 text-xs uppercase tracking-wider">ADD:</label>
                        <input type="text" name="add" id="party-add" value="{{ $generateBill->add }}" placeholder="Address..." class="flex-1 font-medium text-slate-700 text-sm bg-white border border-slate-300 shadow-sm rounded px-3 py-1.5 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div class="flex items-center gap-3">
                        <label class="font-bold text-slate-600 w-12 text-xs uppercase tracking-wider">GST:</label>
                        <input type="text" name="gst" id="party-gst" value="{{ $generateBill->gst }}" placeholder="GST No..." class="flex-1 font-medium text-slate-700 text-sm bg-white border border-slate-300 shadow-sm rounded px-3 py-1.5 focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>
            </div>
            ...
            <div class="w-full lg:w-1/4 bg-white border border-slate-300 shadow-sm rounded-md overflow-hidden flex flex-col">
                ...
                <div class="p-4 flex flex-col gap-4 flex-1 justify-center">
                    <div class="flex flex-col gap-1 text-center border-b border-slate-100 pb-3">
                        ...
                        <input type="text" name="bill_no" value="{{ $generateBill->bill_no }}" placeholder="Auto" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-indigo-700 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full placeholder-slate-400 text-center shadow-sm">
                    </div>
                    <div class="flex flex-col gap-1 text-center">
                        <label class="font-bold text-slate-600 text-[10px] uppercase tracking-wider">Date</label>
                        <input type="date" name="date" value="{{ \Carbon\Carbon::parse($generateBill->date)->format('Y-m-d') }}" required class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-center shadow-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <table class="w-full min-w-[800px] text-sm text-left border border-slate-200">
            ...
            <tbody id="bill-tbody">
                ...
                @foreach($generateBill->items as $index => $item)
                    @php $rowCount++; @endphp
                    <tr class="border-b border-slate-200 hover:bg-slate-50/50 group">
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="text" name="items[{{$rowCount}}][sr_no]" value="{{ $item->sr_no ?? '' }}" oninput="onChalanInput(this)" onblur="fetchChalanDetails(this)" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="text" name="items[{{$rowCount}}][ch_no]" value="{{ $item->ch_no }}" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                        <td class="px-4 py-2 border-r border-slate-200 align-top">
                            <div class="flex flex-col gap-2 details-container mt-1" data-row="{{$rowCount}}" data-detail-count="{{ count($item->details ?? []) - 1 }}">
                                    ...
                                    @foreach($item->details as $dIndex => $detail)
                                        <div class="flex gap-2 items-center">
                                            <div class="relative combo-container w-1/3 shrink-0">
                                                <input type="text" name="items[{{$rowCount}}][details][{{$dIndex}}][bundle]" value="{{ $detail['bundle'] ?? 'bundles' }}" class="combo-input w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center" onclick="this.nextElementSibling.classList.toggle('hidden')" oninput="filterCombo(this)">
                                            ...
                                            <input type="text" name="items[{{$rowCount}}][details][{{$dIndex}}][value]" value="{{ ltrim($detail['value'] ?? '', '#') ? '#' . ltrim($detail['value'], '#') : '#' }}" placeholder="Detail" class="flex-1 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center min-w-0 detail-code-input">
                                            @if($dIndex == 0)
                        ...
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="number" name="items[{{$rowCount}}][pcs]" value="{{ $item->pcs }}" oninput="calculateRowAmount(this)" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="number" name="items[{{$rowCount}}][rate]" step="0.01" value="{{ $item->rate }}" oninput="calculateRowAmount(this)" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-center mt-1">
                        </td>
                        <td class="px-2 py-2 border-r border-slate-200 align-top">
                            <input type="number" name="items[{{$rowCount}}][amount]" step="0.01" value="{{ $item->amount }}" readonly class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-right mt-1 bg-slate-50">
                        </td>
    ...
    <div class="shrink-0 lg:sticky lg:bottom-0 lg:z-20 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.05)]">
        ...
        <div class="p-3 border-t border-slate-200 bg-white flex flex-wrap justify-between gap-3">
            <div class="flex items-center justify-between gap-3 border border-slate-200 px-3 py-1.5 rounded-md bg-slate-50 flex-1 min-w-[180px]">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" name="vatav_percent" value="{{ $generateBill->vatav_percent ?? 5.00 }}" class="w-14 border border-slate-300 py-0.5 px-1 text-right font-bold text-slate-800 bg-white shadow-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 rounded text-sm">
            ...
            <div class="flex items-center justify-between gap-3 border border-slate-200 px-3 py-1.5 rounded-md bg-slate-50 flex-1 min-w-[180px]">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" name="sgst_percent" value="{{ $generateBill->sgst_percent ?? 2.50 }}" class="w-14 border border-slate-300 py-0.5 px-1 text-right font-bold text-slate-800 bg-white shadow-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 rounded text-sm">
            ...
            <div class="flex items-center justify-between gap-3 border border-slate-200 px-3 py-1.5 rounded-md bg-slate-50 flex-1 min-w-[180px]">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" name="cgst_percent" value="{{ $generateBill->cgst_percent ?? 2.50 }}" class="w-14 border border-slate-300 py-0.5 px-1 text-right font-bold text-slate-800 bg-white shadow-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 rounded text-sm">
            ...
            <div class="flex items-center justify-between gap-3 border border-slate-200 px-3 py-1.5 rounded-md bg-slate-50 flex-1 min-w-[180px]">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" name="tds_percent" value="{{ $generateBill->tds_percent ?? 1.00 }}" class="w-14 border border-slate-300 py-0.5 px-1 text-right font-bold text-slate-800 bg-white shadow-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 rounded text-sm">
        ...
        <div class="p-3 border-t border-slate-200 bg-slate-50 flex flex-col lg:flex-row justify-between items-center gap-4">
            ...
            <div class="flex flex-wrap lg:flex-nowrap justify-center gap-2 w-full lg:w-auto">
                ...
                <button type="submit" name="action" value="generate" class="w-[calc(50%-0.25rem)] lg:w-auto px-4 py-2 bg-indigo-600 text-white font-medium hover:bg-indigo-700 transition-colors shadow-sm rounded text-sm whitespace-nowrap">
                    Generate Bill
                </button>
...
<script>
    ...
    initDetailCodeInputs();
    updateGrandTotal(); // Init total on load
</script>
@endsection
