...
<div class="flex w-full h-full">
    <div id="form-container" class="w-full transition-all duration-300 h-full">
        <form action="{{ route('generate-chalans.store') }}" method="POST"
            class="bg-white shadow-sm border border-slate-200 overflow-y-auto h-full flex flex-col">
        @csrf
        <!-- Form Header -->
                ...
                <div class="flex-1 bg-white border border-slate-300 shadow-sm relative rounded-md overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 flex flex-col gap-3 flex-1">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <input type="text" id="party-address" placeholder="Address..." readonly class="flex-1 font-medium text-slate-700 text-sm bg-slate-50 border border-slate-200 shadow-inner rounded px-3 py-1.5 focus:ring-0 cursor-not-allowed">
                        </div>
                        <div class="flex items-center gap-3">
                            <label class="font-bold text-slate-600 w-12 text-xs uppercase tracking-wider">GST:</label>
                            <input type="text" id="party-gst" placeholder="GST No..." readonly class="flex-1 font-medium text-slate-700 text-sm bg-slate-50 border border-slate-200 shadow-inner rounded px-3 py-1.5 focus:ring-0 cursor-not-allowed">
                        </div>
                    </div>
                </div>
                ...
                <div class="w-full lg:w-1/3 bg-white border border-slate-300 shadow-sm rounded-md overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 flex flex-col gap-4 flex-1 justify-center">
                        <div class="flex items-center justify-between gap-3 border-b border-slate-100 pb-3">
                            ...
                            <input type="text" name="chalan_no" placeholder="Auto" class="bg-white border border-slate-300 shadow-sm rounded px-3 py-1.5 text-indigo-700 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-24 text-center placeholder-slate-400">
                        </div>
                        <div class="flex items-center justify-between gap-3">
                            <label class="font-bold text-slate-700 text-sm whitespace-nowrap">Date.-</label>
                            <input type="date" name="date" required value="{{ date('Y-m-d') }}" class="bg-white border border-slate-300 shadow-sm rounded px-3 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-36 text-center">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="p-3 border-t border-slate-200 bg-slate-50 flex flex-col lg:flex-row justify-between items-center gap-4 shrink-0 shadow-[0_-4px_10px_-1px_rgba(0,0,0,0.05)]">
            ...
            <div class="flex flex-col sm:flex-row flex-wrap lg:flex-nowrap justify-center gap-2 w-full lg:w-auto">
                ...
                <button type="button" onclick="clearDetails()" class="w-full sm:w-[calc(50%-0.25rem)] lg:w-auto px-4 py-2 bg-red-50 border border-red-200 text-red-700 font-bold hover:bg-red-100 transition-colors shadow-sm rounded text-sm whitespace-nowrap">
                    Clear Challan
                </button>
                <a href="{{ route('generate-chalans.index') }}" class="w-full sm:w-[calc(50%-0.25rem)] lg:w-auto px-4 py-2 bg-white border border-slate-300 text-slate-700 font-medium hover:bg-slate-50 transition-colors shadow-sm flex items-center justify-center rounded text-sm whitespace-nowrap">
                    Cancel
                </a>
            </div>
        </div>
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-2 border-r border-slate-200 align-top">
                    <div class="flex gap-2">
                        ...
                        <input type="text" name="items[${rowIdCounter}][code]" value="${code}" placeholder="Code" class="w-1/2 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border detail-code-input">
                    </div>
                </td>
                ...
                <td class="px-4 py-2 border-r border-slate-200 align-top"><input type="number" name="items[${rowIdCounter}][rate]" value="${rate}" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-right" oninput="calculateAmount(this)"></td>
                <td class="px-4 py-2 border-r border-slate-200 align-top"><input type="number" name="items[${rowIdCounter}][amount]" value="${amount}" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-right row-amount" readonly></td>
