...
@section('content')
<form action="{{ route('generate-chalans.store') }}" method="POST" class="bg-white shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
    @csrf
    <!-- Form Header -->
        ...
        <div class="flex gap-4">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <select name="party_id" required class="flex-1 font-bold text-slate-800 text-base bg-transparent border-none p-0 focus:ring-0" onchange="updatePartyDetails(this)">
                            ...
                            @foreach($parties as $party)
                                <option value="{{ $party->id }}" data-address="{{ $party->address }}" data-gst="{{ $party->gst_number }}">{{ $party->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex items-center gap-2 mt-2">
                        <label class="font-semibold text-slate-700 w-24 text-sm uppercase">REF CH NO:</label>
                        <input type="text" id="ref_chalan_no" placeholder="0001" class="flex-1 font-medium text-slate-600 text-sm bg-transparent border-b border-slate-300 p-0 focus:ring-0">
                        <button type="button" onclick="fetchChalanDetails()" class="px-2 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold border border-indigo-200">FETCH</button>
                    </div>
                    <div class="flex items-center gap-2 mt-2">
                        <label class="font-semibold text-slate-700 w-12 text-sm uppercase">ADD:</label>
                        <input type="text" id="party-address" placeholder="Add" readonly class="flex-1 font-medium text-slate-600 text-sm bg-transparent border-none p-0 focus:ring-0 cursor-not-allowed">
                    </div>
                    <div class="flex items-center gap-2">
                        <label class="font-semibold text-slate-700 w-12 text-sm uppercase">GST:</label>
                        <input type="text" id="party-gst" placeholder="GST" readonly class="flex-1 font-medium text-slate-600 text-sm bg-transparent border-none p-0 focus:ring-0 cursor-not-allowed">
                    </div>
                </div>
            </div>
            ...
            <div class="w-1/3 bg-white border border-slate-200 p-4 shadow-sm flex flex-col justify-center gap-2">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <input type="date" name="date" required value="{{ date('Y-m-d') }}" class="bg-transparent border-none p-0 text-slate-900 font-bold text-lg focus:ring-0 w-full">
                </div>
                <div class="flex items-center gap-1 mt-1 border-t border-slate-200 pt-2">
                    ...
                    <input type="text" name="gst" placeholder="--" class="bg-transparent border-b border-slate-300 p-0 text-slate-900 font-bold text-sm focus:ring-0 w-full placeholder-slate-400">
                </div>
                <div class="flex items-center gap-1">
                    <span class="font-semibold text-slate-700 text-sm whitespace-nowrap w-16">P.Date :</span>
                    <input type="date" name="payment_date" class="bg-transparent border-b border-slate-300 p-0 text-slate-900 font-bold text-sm focus:ring-0 w-full">
                </div>
                <div class="flex items-center gap-1">
                    <span class="font-semibold text-slate-700 text-sm whitespace-nowrap w-16">P. Dtl :</span>
                    <input type="text" name="payment_detail" placeholder="--" class="bg-transparent border-b border-slate-300 p-0 text-slate-900 font-bold text-sm focus:ring-0 w-full placeholder-slate-400">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="flex-1 overflow-auto p-6">
        ...
        <div class="mt-4">
            <button onclick="addRow()" class="text-sm font-medium text-indigo-600 hover:text-indigo-700 flex items-center gap-1 p-2 bg-indigo-50 border border-indigo-100 shadow-sm transition-colors">
    ...
    <div class="p-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-3 shrink-0">
        ...
        <a href="/" class="px-6 py-2 bg-white border border-slate-300 text-slate-700 font-medium hover:bg-slate-50 transition-colors shadow-sm flex items-center justify-center">
            Cancel
        </a>
    </div>
</form>
        ...
        tr.innerHTML = `
            ...
            <td class="px-4 py-2 border-r border-slate-200 align-top">
                <div class="flex gap-2">
                    ...
                    <input type="text" name="items[${rowCount}][code]" value="${code}" placeholder="Code" class="w-1/2 border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border">
                </div>
            </td>
            ...
            <td class="px-4 py-2 border-r border-slate-200 align-top"><input type="number" name="items[${rowCount}][rate]" step="0.01" value="${rate}" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-right" oninput="calculateAmount(this)"></td>
            <td class="px-4 py-2 border-r border-slate-200 align-top"><input type="number" name="items[${rowCount}][amount]" step="0.01" value="${amount}" class="w-full border-slate-200 p-1.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border text-right row-amount" readonly></td>
