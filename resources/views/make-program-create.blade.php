@extends('layouts.app')
@section('title', 'Add Program: ' . $firm->name . ' - ' . $machine->machine_no)
...
<div class="h-full flex flex-col overflow-hidden bg-slate-50">
    <div class="flex flex-col sm:flex-row items-center justify-between bg-white p-3 sm:p-4 border-b border-slate-200 shadow-sm shrink-0 gap-3">
        <div class="flex items-center gap-3 w-full">
            ...
            <h2 class="text-base sm:text-xl font-bold text-slate-800 uppercase tracking-wider leading-tight whitespace-normal sm:truncate">Add Program: {{ $firm->name }} / {{ $machine->machine_no }}</h2>
        </div>
    </div>

    <div class="flex-1 p-6 overflow-auto custom-scrollbar">
        <form action="{{ route('make-program.program.store', ['firm' => $firm->id, 'machine' => $machine->id]) }}" method="POST" class="max-w-6xl mx-auto">
            @csrf
                ...
                <div class="p-6 flex flex-col gap-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex flex-col gap-1 text-center">
                            ...
                            <select name="ch_no" id="ch_no" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase" data-selected-chalan="{{ old('ch_no') }}">
                    ...
                    <div class="flex flex-col md:flex-row gap-6 w-full">
                        ...
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            ...
                            <input type="text" name="chart" id="chart" value="{{ old('chart') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>

                        <!-- Mtr -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="mtr" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Mtr</label>
                            <input type="number" step="0.01" name="mtr" id="mtr" value="{{ old('mtr') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>

                        <!-- Pcs -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="pcs" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Pcs</label>
                            <input type="number" name="pcs" id="pcs" value="{{ old('pcs') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>

                        <!-- Rs -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="rs" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Rs</label>
                            <input type="number" step="0.01" name="rs" id="rs" value="{{ old('rs') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>

                        <!-- Detail -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="detail" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Detail</label>
                            <input type="text" name="detail" id="detail" value="{{ old('detail') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase" placeholder="Enter Details">
                        </div>

                        <!-- Design Code -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="design_code" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Design Code</label>
                            <input type="text" name="design_code" id="design_code" value="{{ old('design_code') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase" placeholder="Enter Design Code">
                        </div>

                        <!-- Note -->
                        <div class="flex-1 flex flex-col gap-1 text-center">
                            <label for="note" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Note</label>
                            <input type="text" name="note" id="note" value="{{ old('note') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase" placeholder="Enter Note">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
                                ...
                                <select name="process" id="process" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                                    ...
                                    @if(isset($dropdownOptions['process']))
                                        @foreach($dropdownOptions['process'] as $opt)
                                            <option value="{{ $opt->value }}" {{ old('process') == $opt->value ? 'selected' : '' }}>{{ $opt->value }}</option>
                                        @endforeach
                                    @else
                                        <option value="M.W" {{ old('process') == 'M.W' ? 'selected' : '' }}>M.W</option>
                                        <option value="M.C" {{ old('process') == 'M.C' ? 'selected' : '' }}>M.C</option>
                                        <option value="D.C" {{ old('process') == 'D.C' ? 'selected' : '' }}>D.C</option>
                                        <option value="DECO" {{ old('process') == 'DECO' ? 'selected' : '' }}>DECO</option>
                                        <option value="C.W" {{ old('process') == 'C.W' ? 'selected' : '' }}>C.W</option>
                                        <option value="BARST" {{ old('process') == 'BARST' ? 'selected' : '' }}>BARST</option>
                                        <option value="SIROST" {{ old('process') == 'SIROST' ? 'selected' : '' }}>SIROST</option>
                                        <option value="R.D" {{ old('process') == 'R.D' ? 'selected' : '' }}>R.D</option>
                                    @endif
                                </select>
                                ...
                                <select name="work_percent" id="work_percent" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                                    ...
                                    @if(isset($dropdownOptions['work_percent']))
                                        @foreach($dropdownOptions['work_percent'] as $opt)
                                            <option value="{{ $opt->value }}" {{ old('work_percent') == $opt->value ? 'selected' : '' }}>{{ $opt->value }}%</option>
                                        @endforeach
                                    @else
                                        <option value="0" {{ old('work_percent') == '0' ? 'selected' : '' }}>0%</option>
                                        <option value="25" {{ old('work_percent') == '25' ? 'selected' : '' }}>25%</option>
                                        <option value="50" {{ old('work_percent') == '50' ? 'selected' : '' }}>50%</option>
                                        <option value="75" {{ old('work_percent') == '75' ? 'selected' : '' }}>75%</option>
                                        <option value="100" {{ old('work_percent') == '100' ? 'selected' : '' }}>100%</option>
                                    @endif
                                </select>
                        ...
                        <div class="flex flex-col gap-1 text-center">
                            ...
                            <input type="time" name="time" id="time" value="{{ old('time') }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>

                        <!-- Date -->
                        <div class="flex flex-col gap-1 text-center">
                            <label for="date" class="font-bold text-slate-600 text-[10px] uppercase tracking-wider text-left">Date</label>
                            <input type="date" name="date" id="date" value="{{ old('date', date('Y-m-d')) }}" class="bg-white border border-slate-300 rounded px-2 py-1.5 text-slate-900 font-bold text-sm focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 w-full text-left shadow-sm uppercase">
                        </div>
                        ...
                        <div class="flex flex-col gap-1 text-center">
                            ...
                            <label class="flex items-center justify-center gap-2 bg-white border border-slate-300 rounded px-2 py-1.5 h-[34px] cursor-pointer hover:bg-slate-50 transition-colors shadow-sm w-full">
                                ...
                                <input type="checkbox" name="is_live" value="1" class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500" {{ old('is_live', true) ? 'checked' : '' }}>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if(partySelect && chNoSelect) {
            ...
            // Trigger change event on load in case party is pre-selected (e.g. going back or validation error)
        ...
        if (processSelect && workPercentSelect) {
            ...
            function toggleWorkPercent() {
                ...
                if ((subOptions && subOptions.length > 0) || selectedProcess === 'M.W') {
                    ...
                    workPercentSelect.value = ''; // clear value if no sub-options
                }
            }
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (select) {
                            ...
                            // If adding to process, we might need to refresh sub-options tracking, but page reload is fine for complex cases
                            if (column === 'process') {
