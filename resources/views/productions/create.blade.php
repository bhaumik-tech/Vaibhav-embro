                    ...
                    <div class="relative flex-1">
                        <select name="karigar_id" id="karigar_id" required class="w-full border-2 border-slate-800 p-2.5 text-sm font-bold text-slate-800 appearance-none focus:border-indigo-600 focus:ring-0 cursor-pointer bg-white text-center uppercase tracking-widest">
                            ...
                            @foreach($karigars as $karigar)
                                @php
                                    $mNames = [];
                                    if ($karigar->machine1) $mNames[] = $karigar->machine1->machine_no;
                                    if ($karigar->machine2) $mNames[] = $karigar->machine2->machine_no;
                                    if ($karigar->machine3) $mNames[] = $karigar->machine3->machine_no;
                                    $mSuffix = !empty($mNames) ? ' (' . implode(', ', $mNames) . ')' : '';
                                @endphp
                                <option value="{{ $karigar->id }}">{{ $karigar->name }}{{ $mSuffix }}</option>
                            @endforeach
                        </select>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function applyMachineType(machineId, selectTypeElement) {
            if (!machineId || !selectTypeElement) return;
            const machine = machinesData.find(m => m.id == machineId);
            if (machine && machine.top_dup) {
                const val = machine.top_dup.toLowerCase();
                // Find matching option
                const optionExists = Array.from(selectTypeElement.options).some(opt => opt.value === val);
                if (optionExists) {
                    selectTypeElement.value = val;
                    selectTypeElement.dispatchEvent(new Event('change', { bubbles: true }));
                    autoSwitchMate(selectTypeElement);
                }
            }
        }
        ...
        machinesContainer.addEventListener('change', function(e) {
            ...
            // Auto fill top/dup based on machine selection
            if (e.target.tagName === 'SELECT' && (e.target.name === 'machine_1_id' || e.target.name === 'machine_2_id' || e.target.name === 'machine_3_id' || (e.target.name && e.target.name.startsWith('m_id[')))) {
                const machineId = e.target.value;
                const machineRow = e.target.closest('.machine-row');
                if (machineRow) {
                    const typeSelect = machineRow.querySelector('select[name="m1_type"], select[name="m2_type"], select[name="m3_type"], select[name="m_type[]"]');
                    if (typeSelect) {
                        applyMachineType(machineId, typeSelect);
                    }
                }
            }
        });
        ...
        karigarSelect.addEventListener('change', function() {
            ...
            if (karigar) {
                if (karigar.machine_1_id) {
                    m1Select.value = karigar.machine_1_id;
                    m1Select.dispatchEvent(new Event('change', { bubbles: true }));
                }
                if (karigar.machine_2_id) {
                    m2Select.value = karigar.machine_2_id;
                    const m2cb = document.querySelector('[name="m2_active"]');
                    if(m2cb) m2cb.checked = true;
                    m2Select.dispatchEvent(new Event('change', { bubbles: true }));
                } else {
                    const m2cb = document.querySelector('[name="m2_active"]');
                    if(m2cb) m2cb.checked = false;
                }
                
                if (karigar.machine_3_id) {
                    m3Select.value = karigar.machine_3_id;
                    const m3cb = document.querySelector('[name="m3_active"]');
                    if(m3cb) m3cb.checked = true;
                    m3Select.dispatchEvent(new Event('change', { bubbles: true }));
                } else {
                    const m3cb = document.querySelector('[name="m3_active"]');
                    if(m3cb) m3cb.checked = false;
                }
            }
        });
    });
        ...
        if (savedDraft) {
            try {
                ...
                setTimeout(() => {
                    ...
                    // Also auto-apply machine types based on selected machines (overrides draft type if it conflicts)
                    const machineSelects = form.querySelectorAll('select[name^="machine_"], select[name^="m_id["]');
                    machineSelects.forEach(select => {
                        const machineId = select.value;
                        const machineRow = select.closest('.machine-row');
                        if (machineId && machineRow) {
                            const typeSelect = machineRow.querySelector('select[name$="type"], select[name$="type[]"]');
                            if (typeSelect) {
                                applyMachineType(machineId, typeSelect);
                            }
                        }
