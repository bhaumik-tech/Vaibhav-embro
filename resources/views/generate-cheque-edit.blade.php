@extends('layouts.app')
@section('title', 'Edit Generated Cheque')
...
<div class="bg-slate-50 shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
    <div class="flex-1 overflow-auto p-8">
        <div class="max-w-2xl mx-auto bg-white border border-slate-400 p-6 shadow-sm flex flex-col gap-5">
            ...
            <form action="{{ route('generate-cheque.update', $cheque->id) }}" method="POST" class="flex flex-col gap-5">
                @csrf
                @method('PUT')
            ...
            <div class="flex gap-4 justify-between">
                <label class="w-1/3 border border-slate-300 p-2.5 flex items-center justify-center gap-3 cursor-pointer hover:bg-slate-50 transition-colors bg-white">
                    <input type="checkbox" name="is_ac_payee" value="1" class="w-5 h-5 text-green-500 border-slate-300 rounded focus:ring-green-500 focus:ring-offset-0 cursor-pointer" {{ $cheque->is_ac_payee ? 'checked' : '' }}>
                    <span class="text-sm font-bold text-slate-700">A/c payee</span>
                </label>
                <input type="date" name="date" value="{{ \Carbon\Carbon::parse($cheque->date)->format('Y-m-d') }}" required class="w-1/3 border border-slate-300 p-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 font-medium text-slate-700 text-center">
            </div>

            <!-- Row 2: Payee name -->
            <div class="relative w-full" id="payee-combo-container">
                <input type="text" name="payee_name" id="payee-input" value="{{ $cheque->payee_name }}" placeholder="Payee name (select from dropdown or type name)" autocomplete="off" 
            ...
            <div class="flex gap-4 items-stretch">
                ...
                <div class="flex-1 flex flex-col justify-between">
                    ...
                    <div class="relative w-full">
                        <select name="firm_id" class="w-full border border-slate-300 p-2.5 text-sm font-bold text-slate-700 appearance-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 cursor-pointer bg-white text-center">
                            ...
                            @foreach($firms as $firm)
                                <option value="{{ $firm->id }}" {{ $cheque->firm_id == $firm->id ? 'selected' : '' }}>{{ $firm->name }}</option>
                            @endforeach
                        </select>
                        <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-slate-400">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    ...
                    <!-- Remark / note -->
                    <input type="text" name="remark" value="{{ $cheque->remark }}" placeholder="Remark/ note" class="w-full border border-slate-300 p-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 placeholder-slate-500 font-bold text-center">
                </div>
                ...
                <div class="flex-1 flex flex-col gap-4">
                    ...
                    <div class="flex gap-4">
                        ...
                        <div class="relative flex-1">
                            <select name="bill_no" class="w-full border border-slate-300 p-2.5 text-sm font-bold text-slate-700 appearance-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 cursor-pointer bg-white text-center">
                                <option value="" {{ empty($cheque->bill_no) ? 'selected' : '' }}>--</option>
                                <option value="101" {{ $cheque->bill_no == '101' ? 'selected' : '' }}>101</option>
                                <option value="102" {{ $cheque->bill_no == '102' ? 'selected' : '' }}>102</option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-slate-400">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                            </div>
                    ...
                    <!-- Amount -->
                    <input type="number" step="0.01" name="amount" value="{{ $cheque->amount }}" required placeholder="Amount" class="w-full border border-slate-300 p-2.5 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 placeholder-slate-500 font-bold text-center">
