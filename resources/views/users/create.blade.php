        ...
        <div class="w-full max-w-3xl bg-white border border-slate-300 shadow-sm p-8">
            ...
            <form action="{{ route('users.store') }}" method="POST" class="flex flex-col gap-6">
                ...
                <div>
                    ...
                    <div class="border border-slate-300 bg-white overflow-hidden shadow-sm">
                        ...
                        <div id="perm-table-{{ $fKey }}" class="perm-table-container {{ $fKey === 'global' ? '' : 'hidden' }}">
                            <div class="bg-indigo-50 border-b border-slate-200 px-4 py-2 flex justify-between items-center text-xs font-bold text-indigo-800 uppercase tracking-widest">
                                <span>Permissions for: {{ $fKey === 'global' ? 'Global (Fallback)' : $firms->firstWhere('id', $fKey)->name }}</span>
                                <div class="flex gap-2">
                                    <button type="button" onclick="selectAllPerms('{{ $fKey }}')" class="bg-indigo-600 text-white px-2 py-1 rounded hover:bg-indigo-700 transition-colors shadow-sm">Select All</button>
                                    <button type="button" onclick="clearAllPerms('{{ $fKey }}')" class="bg-red-500 text-white px-2 py-1 rounded hover:bg-red-600 transition-colors shadow-sm">Clear All</button>
                                </div>
                            </div>
                                    ...
                                    <tbody class="divide-y divide-slate-100">
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors">
                                            ...
                                            <td class="p-3 text-center border-r border-slate-200 hover:bg-blue-50/50 transition-colors">
                                                <input type="checkbox" name="page_permissions[{{ $fKey }}][{{ $key }}][]" value="view" class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-500 cursor-pointer">
                                            </td>
                                            <td class="p-3 text-center border-r border-slate-200 hover:bg-indigo-50/50 transition-colors">
                                                <input type="checkbox" name="page_permissions[{{ $fKey }}][{{ $key }}][]" value="edit" class="w-4 h-4 text-indigo-600 border-slate-300 rounded focus:ring-indigo-500 cursor-pointer">
                                            </td>
                                            <td class="p-3 text-center hover:bg-red-50/50 transition-colors">
                                                <input type="checkbox" name="page_permissions[{{ $fKey }}][{{ $key }}][]" value="remove" class="w-4 h-4 text-red-600 border-slate-300 rounded focus:ring-red-500 cursor-pointer">
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                ...
                <script>
                    ...
                    function selectAllPerms(fKey) {
                        const container = document.getElementById('perm-table-' + fKey);
                        container.querySelectorAll('input[type="checkbox"]').forEach(cb => cb.checked = true);
                    }
                    
                    function clearAllPerms(fKey) {
                        const container = document.getElementById('perm-table-' + fKey);
