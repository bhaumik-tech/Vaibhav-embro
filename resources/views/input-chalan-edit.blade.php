...
<form action="{{ route('input-chalan.update', $inputChalan) }}" method="POST" class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden h-full flex flex-col">
            ...
            <div>
                ...
                <select name="party_id" id="party_id" onchange="filterFirms()" required class="w-full border-slate-300 rounded-md shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm p-2 border font-semibold text-slate-700">
                    ...
                    @foreach($parties as $party)
                        <option value="{{ $party->id }}" data-firms="{{ $party->firms->pluck('id')->join(',') }}" {{ $inputChalan->party_id == $party->id ? 'selected' : '' }}>{{ $party->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
    ...
    <div class="p-4 border-t border-slate-200 bg-slate-50 flex justify-end gap-3">
        <a href="{{ request('return_to', route('register.index')) }}" class="px-4 py-2 border border-slate-300 rounded-lg text-sm font-medium text-slate-700 hover:bg-white transition-colors shadow-sm">
            Cancel
        </a>
