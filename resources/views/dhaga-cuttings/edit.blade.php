@extends('layouts.app')
@section('title', 'Edit Dhaga Cutting')
...
<div class="h-full flex flex-col max-w-4xl mx-auto w-full">
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 mb-6 shrink-0">
        <a href="{{ route('dhaga-cuttings.index', ['person_id' => $dhagaCutting->person_id]) }}" class="h-10 w-10 bg-white border border-slate-300 flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-sm">
    ...
    <div class="bg-white border border-slate-300 shadow-sm p-8 flex-1">
        <form action="{{ route('dhaga-cuttings.update', $dhagaCutting) }}" method="POST" class="flex flex-col h-full">
            @csrf
            @method('PUT')
            ...
            <div class="flex gap-4 mb-8">
                <div class="flex-1">
                    <select name="person_id" required class="w-full border border-slate-300 p-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50 font-bold text-slate-800 text-center uppercase tracking-widest cursor-pointer">
                        ...
                        @foreach($people as $person)
                            <option value="{{ $person->id }}" {{ old('person_id', $dhagaCutting->person_id) == $person->id ? 'selected' : '' }}>
                                {{ $person->person_name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="w-48 shrink-0">
                    <input type="date" name="date" required value="{{ old('date', $dhagaCutting->date) }}"
                        class="w-full border border-slate-300 p-3 text-sm focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 bg-slate-50 font-bold text-slate-800 text-center uppercase tracking-widest cursor-pointer">
                </div>
            </div>
            ...
            <div class="flex-1 space-y-3 mb-8">
                @foreach($fixedRates as $rate)
                    @php
                        $pieces = isset($itemsMap[$rate]) && $itemsMap[$rate]['pieces'] > 0 ? $itemsMap[$rate]['pieces'] : '';
                        $amount = isset($itemsMap[$rate]) && $itemsMap[$rate]['amount'] > 0 ? $itemsMap[$rate]['amount'] : '';
                    @endphp
                    <div class="flex items-center gap-4">
                        ...
                        <input type="number" step="0.01" name="items[{{ $itemIndex }}][pieces]" value="{{ $pieces }}" placeholder="Piece"
                        ...
                        <input type="number" step="0.01" name="items[{{ $itemIndex }}][amount]" value="{{ $amount }}" placeholder="Amount" readonly
                ...
                <!-- Custom Rate Row -->
                @php
                    $customRate = isset($itemsMap['Custom']) && $itemsMap['Custom']['rate_value'] > 0 ? $itemsMap['Custom']['rate_value'] : '';
                    $customPieces = isset($itemsMap['Custom']) && $itemsMap['Custom']['pieces'] > 0 ? $itemsMap['Custom']['pieces'] : '';
                    $customAmount = isset($itemsMap['Custom']) && $itemsMap['Custom']['amount'] > 0 ? $itemsMap['Custom']['amount'] : '';
                @endphp
                <div class="flex items-center gap-4 pt-4 mt-4 border-t border-slate-200">
                    <div class="w-32 shrink-0 flex">
                        ...
                        <input type="number" step="0.01" name="items[{{ $itemIndex }}][rate_value]" value="{{ $customRate }}" placeholder="Custom"
                    ...
                    <input type="number" step="0.01" name="items[{{ $itemIndex }}][pieces]" value="{{ $customPieces }}" placeholder="Piece"
                        ...
                        oninput="calculateCustomRow(this)">
                    <input type="number" step="0.01" name="items[{{ $itemIndex }}][amount]" value="{{ $customAmount }}" placeholder="Amount" readonly
                ...
                <div class="flex items-center gap-4 pt-2">
                    ...
                    <input type="text" id="total_pieces" name="total_pieces" value="{{ $dhagaCutting->total_pieces > 0 ? $dhagaCutting->total_pieces : '' }}" placeholder="Piece" readonly
                        class="flex-1 border border-slate-300 p-3 text-sm bg-slate-100 font-bold text-slate-800 text-center cursor-not-allowed">
                    <input type="text" id="total_amount" name="total_amount" value="{{ $dhagaCutting->total_amount > 0 ? $dhagaCutting->total_amount : '' }}" placeholder="Amount" readonly
                        class="flex-1 border border-slate-300 p-3 text-sm bg-slate-100 font-bold text-slate-800 text-center cursor-not-allowed">
                </div>
            </div>
            ...
            <div class="mt-auto border-t border-slate-200 pt-6 flex flex-wrap items-center justify-between gap-4">
                <div class="flex-1 min-w-[200px]">
                    <input type="text" name="remark_note" value="{{ old('remark_note', $dhagaCutting->remark_note) }}" placeholder="Remark / Note"
                ...
                <div class="flex items-center gap-4">
                    <label class="flex items-center justify-center bg-white text-slate-700 border border-slate-300 px-6 py-3 text-sm font-bold hover:bg-slate-50 transition-colors shadow-sm uppercase tracking-widest cursor-pointer select-none">
                        <input type="checkbox" name="is_highlighted" value="1" {{ $dhagaCutting->is_highlighted ? 'checked' : '' }} class="mr-2 text-indigo-600 focus:ring-indigo-500"> Highlight
                    </label>
                    ...
                    <a href="{{ route('dhaga-cuttings.index', ['person_id' => $dhagaCutting->person_id]) }}" class="bg-white text-slate-700 border border-slate-300 px-6 py-3 text-sm font-bold hover:bg-slate-50 transition-colors shadow-sm uppercase tracking-widest flex items-center justify-center">
                        Cancel
                    </a>
                </div>
            </div>
