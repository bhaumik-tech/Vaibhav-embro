@extends('layouts.app')
@section('title', 'Check Status: ' . $firm->name)

@section('content')
<div class="h-full flex flex-col gap-4 overflow-hidden">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-4 shrink-0">
        <a href="{{ route('check-status') }}" class="h-10 w-10 bg-white border border-slate-300 flex items-center justify-center text-slate-500 hover:bg-slate-50 hover:text-indigo-600 transition-colors shadow-sm shrink-0">
            ...
            <table class="w-full text-left border-collapse whitespace-nowrap min-w-[1200px]" id="statusTable">
                <thead class="sticky top-0 bg-slate-100 shadow-sm z-10">
                    <tr class="border-b border-slate-300">
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200 w-16 text-center">Sr.No</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Date/Time</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200 text-indigo-700">Machine</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Party</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Ch.No</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Chart</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Design Code</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200">Detail</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200 text-right">Pcs</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest border-r border-slate-200 text-center">Process</th>
                        <th class="p-4 text-[11px] font-black text-slate-600 uppercase tracking-widest text-center">Work (%)</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse($programs->sortBy('machine.machine_no') as $index => $program)
                        ...
                        <tr style="background-color: {{ $rowBg }};" class="transition-colors hover:brightness-95 search-row">
                            ...
                            <td class="p-4 text-sm font-bold text-slate-800 border-r border-slate-100">
                            ...
                            <td class="p-4 text-sm font-black text-indigo-700 border-r border-slate-100 bg-indigo-50/30 uppercase searchable">{{ $program->machine->machine_no ?? '-' }}</td>
                            <td class="p-4 text-sm font-bold text-slate-800 border-r border-slate-100 searchable">{{ $program->party->name ?? '-' }}</td>
                            <td class="p-4 text-sm font-bold text-slate-700 border-r border-slate-100 searchable">{{ $program->ch_no }}</td>
                            <td class="p-4 text-sm font-medium text-slate-600 border-r border-slate-100 searchable">{{ $program->chart }}</td>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="p-8 text-center text-slate-400 font-bold uppercase tracking-widest text-sm bg-slate-50">
                                No active programs found for this firm.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
